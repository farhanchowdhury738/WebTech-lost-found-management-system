                    ...
                    <details>
                        ...
                        <form action="../Controller/adminUser.php" method="post" style="min-width:250px;margin-top:8px">
                            ...
                            <div class="field"><label>Name</label><input name="name"
                                    value="<?php echo htmlspecialchars($u['name']); ?>"></div>
                            <div class="field"><label>Email</label><input type="email" name="email"
                                    value="<?php echo htmlspecialchars($u['email']); ?>"></div>
                            <div class="field"><label>Phone</label><input name="phone"
                                    value="<?php echo htmlspecialchars($u['phone']); ?>"></div>
                            <div class="field"><label>Role</label><select name="role">
                                    <option value="user" <?php echo $u['role'] === 'user' ? 'selected' : ''; ?>>User</option>
                                    <option value="admin" <?php echo $u['role'] === 'admin' ? 'selected' : ''; ?>>Admin</option>
                                </select></div>
                            <div class="field"><label>Status</label><select name="status">
                                    <option value="active" <?php echo $u['status'] === 'active' ? 'selected' : ''; ?>>Active
                                    </option>
                                    <option value="blocked" <?php echo $u['status'] === 'blocked' ? 'selected' : ''; ?>>Blocked
                                    </option>
                                </select></div>
                            <button class="btn small">Update</button>
                        </form>
                    </details>
                    <?php if ($u['id'] != $_SESSION['loggedInUserId']): ?>
                        <form action="../Controller/adminUser.php" method="post" style="margin-top:8px"><input type="hidden"
                                name="action" value="delete"><input type="hidden" name="id"
                                value="<?php echo $u['id']; ?>"><button class="btn danger small"
                                onclick="return confirm('Delete this user? Their items and claims will also be removed.')">Delete</button>
                        </form><?php else: ?><span class="muted">Current admin</span><?php endif; ?>
                </td>
            </tr>
        <?php endwhile; ?>
    </table>
